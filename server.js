 ...
 */
/**
 * Escape LIKE wildcards in user input so underscores (extremely common in
 * tags, e.g. "tenma_maemi") and percent signs match literally instead of
 * acting as SQL LIKE wildcards. Paired with `ESCAPE '\'` in the query.
...
/**
 * Parse the images-panel search box into SQL conditions over a
 * `staging_images si` row. Tokens are whitespace-separated and AND-ed
 * together (booru-style). Per-token grammar:
 *
 *   foo        image has a tag whose full "category:name" (or bare name
 *              for general tags) CONTAINS foo   (case-insensitive)
 *   =foo       exact full-tag match
 *   foo*       wildcard ( * -> % ), anchored to the whole tag string
 *   file:foo   filename / path CONTAINS foo
 *   -<any>     negate any of the above (image must NOT match)
 *
 * Returns { conditions: string[], params: any[] } to splice into a WHERE.
 * An empty / whitespace-only query returns no conditions.
 */
function buildStagingSearchWhere(search) {
  const conditions = [];
  const params = [];
  ...
  for (let tok of tokens) {
    ...
    if (tok.toLowerCase().startsWith('file:')) {
      ...
      const cond =
        "(LOWER(COALESCE(si.filename, '')) LIKE ? ESCAPE '\\' " +
        "OR LOWER(COALESCE(si.image_path, '')) LIKE ? ESCAPE '\\')";
    ...
    let pattern;
    if (tok.includes('*')) {
      // wildcard: escape LIKE specials, then translate * -> % (anchored)
      pattern = escapeLike(tok).replace(/\*/g, '%').toLowerCase();
    } else if (exact) {
...
async function scanStagingDirectory(limit = 50, offset = 0, sort = 'newest', uploadFilter = 'all', search = '') {
  let orderBy;
  switch (sort) {
  ...
  // Build the WHERE from the upload-state filter + the search box. Both the
  // COUNT and the SELECT use the same clause + params so hasMore/total stay
  // consistent with what's actually returned. The table is aliased `si` so
  // the search subqueries can reference si.id / si.filename unambiguously.
  const conditions = [];
  const params = [];

  if      (uploadFilter === 'pending')  conditions.push('(si.booru_post_id IS NULL OR si.booru_post_id = -1)');
  else if (uploadFilter === 'uploaded') conditions.push('si.booru_post_id >= 0');

  const searchWhere = buildStagingSearchWhere(search);
  conditions.push(...searchWhere.conditions);
  params.push(...searchWhere.params);

  const whereClause = conditions.length ? 'WHERE ' + conditions.join(' AND ') : '';

  const total = db.prepare(
    `SELECT COUNT(*) AS c FROM staging_images si ${whereClause}`
  ).get(...params).c;

  const rows = db.prepare(`
    SELECT
      si.id, si.json_path, si.image_path, si.filename, si.source_url, si.image_hash,
      si.media_type, si.pool_id, si.pool_index,
      si.booru_upload_id, si.booru_media_asset_id, si.booru_post_id,
      si.timestamp, si.tag_count
...
app.get('/api/staging/images', async (req, res) => {
  try {
    ...
    const search       = (req.query.search || '').toString().slice(0, 500);

    const result = await scanStagingDirectory(limit, offset, sort, uploadFilter, search);
    res.json(result);
  } catch (error) {
